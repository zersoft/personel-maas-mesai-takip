                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                foreach ($fazlaMesailer as $fm):
                                    ...
                                    $toplamTutar += (float)$fm['tutar'] = (float)$fm['saat'] * (float)$fm['saat_ucreti'];
                                ?>
                                    <tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Personel seçildiğinde saat ücretini otomatik getir
        const personelSelect = document.querySelector('#fazlaMesaiEkleModal select[name="personel_id"]');
        if (personelSelect) {
            personelSelect.addEventListener('change', function() {
                ...
                if (ucret !== null && ucret !== '') {
                    // Ham değeri sakla
                    if (saatUcretiRaw) saatUcretiRaw.value = ucret;

                    // Gösterilecek değeri formatla (Noktayı virgüle çevir)
                    if (saatUcreti) {
                        // Veritabanından gelen 100.00 formatını 100,00 formatına çevir
                        saatUcreti.value = ucret.toString().replace('.', ',');
                    }
                }
            });
        }
